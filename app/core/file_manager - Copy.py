...
def _merge_preserving_schema(base: Any, edited: Any, path: str = "$") -> Any:
    ...
    if isinstance(base, list) and isinstance(edited, list):
        # If this is an inventory-like UE array, edited data is already schema-correct
        # (it originated from uesave to-json, and we clone existing entries when appending).
        # Returning edited directly avoids expensive deep-merges over thousands of elements.
        allow_grow = (
            'PlayerItems_0' in path
            or 'PlayerItem_0' in path
            or ('CharacterItem_0' in path and 'PlayerItems' in path)
        )
        if allow_grow and len(edited) >= len(base):
            return edited
    
        out = list(base)
        limit = min(len(base), len(edited))
        for j in range(limit):
            out[j] = _merge_preserving_schema(base[j], edited[j], f"{path}[{j}]")
    
        # Never shrink arrays. If allow_grow and edited is longer, append.
        if allow_grow and len(edited) > len(base):
            out.extend(edited[len(base):])
        return out
...
class FileManager:
    ...
    def load_file(path: str) -> dict:
        ...
        # Track source .sav for reliable re-save cycles
        try:
            data.setdefault('_meta', {})
            data['_meta']['src_sav'] = str(p.resolve())
        except Exception:
            pass
        return data
    ...
    def save_sav(
        ...
        # Prefer the source .sav recorded when the file was loaded (fixes 'first edit works, second doesn't').
        try:
            meta_src = (edited_data.get('_meta') or {}).get('src_sav')
        except Exception:
            meta_src = None
        if (not src_path) and meta_src and os.path.isfile(str(meta_src)):
            src_path = str(meta_src)

        # normalize src_path; fall back only if the path exists
        if src_path:
        ...
        # 3) optional debug copy (can be extremely large for big inventories)
        if os.environ.get('LOP_WRITE_MERGED_JSON') == '1':
